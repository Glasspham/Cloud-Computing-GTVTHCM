...
class DashboardService {
  async getDashboardStats(): Promise<DashboardStats> {
    try {
      const response = await httpClient.get<DashboardResponse>("/statistics/dashboard");
  ...
  async getYearlyStats(year: number): Promise<MonthlyStats[]> {
    try {
      const response = await httpClient.get<YearlyStatsResponse>(`/statistics/static-year?year=${year}`);
  ...
  async getLocationStats(year: number): Promise<LocationStats[]> {
    try {
      const response = await httpClient.get<LocationStatsResponse>(`/statistics/static-location-year?year=${year}`);
